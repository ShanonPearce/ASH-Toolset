# -*- coding: utf-8 -*-
"""
Created on Fri Aug 18 23:20:14 2023

@author: Shanon
"""


# import packages
from os.path import join as pjoin
import os
from pathlib import Path
import logging
from datetime import date
from ash_toolset import constants as CN
import numpy as np
from ash_toolset import helper_functions as hf
import dearpygui.dearpygui as dpg
from datetime import datetime, timedelta

today = str(date.today())

logger = logging.getLogger(__name__)
log_info=1






 
def write_ash_e_apo_config(primary_path, hpcf_dict, brir_meta_dict, audio_channels,  spatial_res=1, upmix_method=CN.UPMIXING_METHODS[0], side_delay=0, rear_delay=0, gui_logger=None):
    """
    Function creates equalizer APO configuration file to perform relevant convolution commands. This file needs to be loaded into config.txt
    :param primary_path: string, base path to save files to
    :param hpcf_dict: dict, dictionary containing hpcf related metadata
    :param brir_meta_dict: dict, dictionary containing brir related metadata
    :param primary_path: string, selected audio channel configuration
    :param spatial_res: int, spatial resolution, 0= low, 1 = moderate, 2 = high, 3 = full
    :param gain_adjustment: float, a value to adjust gain with
    :param upmix_method: int, upmixing method if applicable
    :return: float containing overall gain
    """ 
    out_file=''
    preamp=0.0
    
    if upmix_method == CN.UPMIXING_METHODS[0]:
        gain_adjust_upmix = 0.8128
        upmix_factor_front=0.5
        upmix_factor_side=0.3 
        upmix_factor_rear=0.3
        upmix_factor_center=0.2
        
    else:
        gain_adjust_upmix = 0.6237
        upmix_factor_front=1.0
        upmix_factor_side=0.7 
        upmix_factor_rear=0.5
        upmix_factor_center=0.7
    # Apply gain adjustment and round to two decimal places
    upmix_factor_front = round(upmix_factor_front * gain_adjust_upmix, 2)
    upmix_factor_side = round(upmix_factor_side * gain_adjust_upmix, 2)
    upmix_factor_rear = round(upmix_factor_rear * gain_adjust_upmix, 2)
    upmix_factor_center = round(upmix_factor_center * gain_adjust_upmix, 2)
    
    try:
        #get variables from dicts
        enable_hpcf_conv = hpcf_dict.get('enable_conv')
        brand = hpcf_dict.get('brand')
        headphone = hpcf_dict.get('headphone')
        sample = hpcf_dict.get('sample')
        if not headphone or headphone == '' or not sample or sample == '':
            enable_hpcf_conv=False #dont proceed with hpcf convolution if no headphone or sample provided
        
        enable_brir_conv = brir_meta_dict.get('enable_conv')
        brir_set_folder=brir_meta_dict.get('brir_set_folder')
        brir_set_name=brir_meta_dict.get('brir_set_name')
        if not brir_set_name or brir_set_name == '' or not brir_set_folder or brir_set_folder == '':
            enable_brir_conv=False #dont proceed with brir convolution if no set name provided
        
        mute_fl=brir_meta_dict.get('mute_fl')
        mute_fr=brir_meta_dict.get('mute_fr')
        mute_c=brir_meta_dict.get('mute_c')
        mute_sl=brir_meta_dict.get('mute_sl')
        mute_sr=brir_meta_dict.get('mute_sr')
        mute_rl=brir_meta_dict.get('mute_rl')
        mute_rr=brir_meta_dict.get('mute_rr')
        gain_oa=brir_meta_dict.get('gain_oa')
        gain_fl=brir_meta_dict.get('gain_fl')
        gain_fr=brir_meta_dict.get('gain_fr')
        gain_c=brir_meta_dict.get('gain_c')
        gain_sl=brir_meta_dict.get('gain_sl')
        gain_sr=brir_meta_dict.get('gain_sr')
        gain_rl=brir_meta_dict.get('gain_rl')
        gain_rr=brir_meta_dict.get('gain_rr')
        if mute_fl == True:
            gain_fl=-100
        if mute_fr == True:
            gain_fr=-100
        if mute_c == True:
            gain_c=-100
        if mute_sl == True:
            gain_sl=-100
        if mute_sr == True:
            gain_sr=-100
        if mute_rl == True:
            gain_rl=-100
        if mute_rr == True:
            gain_rr=-100
            
        elev_fl=brir_meta_dict.get('elev_fl')
        elev_fr=brir_meta_dict.get('elev_fr')
        elev_c=brir_meta_dict.get('elev_c')
        elev_sl=brir_meta_dict.get('elev_sl')
        elev_sr=brir_meta_dict.get('elev_sr')
        elev_rl=brir_meta_dict.get('elev_rl')
        elev_rr=brir_meta_dict.get('elev_rr')
        
        azim_fl=brir_meta_dict.get('azim_fl')
        azim_fr=brir_meta_dict.get('azim_fr')
        azim_c=brir_meta_dict.get('azim_c')
        azim_sl=brir_meta_dict.get('azim_sl')
        azim_sr=brir_meta_dict.get('azim_sr')
        azim_rl=brir_meta_dict.get('azim_rl')
        azim_rr=brir_meta_dict.get('azim_rr')
        

        #find file names for desired brirs
        brir_set_formatted = brir_set_folder.replace(" ", "_")
        brirs_path = pjoin(primary_path, CN.PROJECT_FOLDER_BRIRS, brir_set_formatted)
        
        brir_name_wav_fl = 'BRIR' + '_E' + str(elev_fl) + '_A' + str(azim_fl) + '.wav'
        brir_name_wav_fr = 'BRIR' + '_E' + str(elev_fr) + '_A' + str(azim_fr) + '.wav'
        brir_name_wav_c = 'BRIR' + '_E' + str(elev_c) + '_A' + str(azim_c) + '.wav'
        brir_name_wav_sl = 'BRIR' + '_E' + str(elev_sl) + '_A' + str(azim_sl) + '.wav'
        brir_name_wav_sr = 'BRIR' + '_E' + str(elev_sr) + '_A' + str(azim_sr) + '.wav'
        brir_name_wav_rl = 'BRIR' + '_E' + str(elev_rl) + '_A' + str(azim_rl) + '.wav'
        brir_name_wav_rr = 'BRIR' + '_E' + str(elev_rr) + '_A' + str(azim_rr) + '.wav'
  
  
        #find brand name and file name for headphone - search through hpcf folder
        # Ensure safe strings
        headphone = headphone or ""
        sample = sample or ""
        brand = brand or ""
        hpcf_name_formatted = headphone.replace(" ", "_")
        sample_name_formatted = sample.replace(" ", "_")
        brand_formatted = brand.replace(" ", "_")
        hpcf_name_wav = hpcf_name_formatted + '_' + sample_name_formatted + '.wav'
        hpcf_name_wav = hf.sanitize_filename(hpcf_name_wav)#sanitize file name in case of invalid windows characters
        hpcf_name_wav = hf.format_name_for_file(hpcf_name_wav)#safety check to replace space with underscore
            
        #hpcf EAPO config path
        output_config_path = pjoin(primary_path, CN.PROJECT_FOLDER_CONFIGS)
    
        out_file_name = 'ASH_Toolset_Config.txt'
        out_file = pjoin(output_config_path, out_file_name)
        
        #create dir if doesnt exist
        output_file = Path(out_file)
        output_file.parent.mkdir(exist_ok=True, parents=True)

        #relative directories
        conv_hpcf_command = 'Convolution: ..\\HpCFs\\FIRs\\'+brand_formatted+'\\'+ hpcf_name_wav
        conv_brir_rel_dir = 'Convolution: ..\\BRIRs\\'+brir_set_formatted+'\\'
   
        #use overall gain as basis for preamp, user defined or auto adjusted
        preamp=preamp+gain_oa

        # Clamp the value: max(MIN, min(VALUE, MAX))
        preamp = max(CN.MIN_GAIN, min(preamp, CN.MAX_GAIN))
        # Round to one decimal place for E-APO compatibility
        preamp = round(preamp, 1)

        # Log a warning if the value was originally outside your logic's range
        if preamp >= CN.MAX_GAIN or preamp <= CN.MIN_GAIN:
            log_string = f"Preamp clamped to safety limit: {preamp} dB"
            hf.log_with_timestamp(log_string=log_string)

        
        #write to txt file
        with open(out_file, 'w') as f:
            #metadata
            f.write('# HpCF and BRIR convolution configuration for Equalizer APO')
            f.write('\n')
            f.write('# Headphone: ' + headphone)
            f.write('\n')
            f.write('# Sample: ' + sample)
            f.write('\n')
            f.write('# BRIR Set: ' + brir_set_name)
            f.write('\n')
            f.write('# Dated: ' + today)
            f.write('\n')
            f.write('\n')
            
            #section to apply gains
            f.write('Channel: ALL')
            f.write('\n')
            f.write('Preamp: '+ str(preamp) +' dB')#also adjust gain
            f.write('\n')
            f.write('\n')
            
            #upmixing if applicable
            if audio_channels == '2.0 Stereo Upmix to 7.1':
                if upmix_method == 'Method A':#basic method
                    copy_string = 'Copy: L='+str(upmix_factor_front)+'*L R='+str(upmix_factor_front)+'*R C='+str(upmix_factor_center)+'*L+'+str(upmix_factor_center)+'*R SUB=0 RL='+str(upmix_factor_rear)+'*L RR='+str(upmix_factor_rear)+'*R SL='+str(upmix_factor_side)+'*L SR='+str(upmix_factor_side)+'*R'
                else:#center and side separation
                    copy_string = 'Copy: L='+str(upmix_factor_front)+'*L R='+str(upmix_factor_front)+'*R C='+str(upmix_factor_center)+'*L+'+str(upmix_factor_center)+'*R SUB=0 RL='+str(upmix_factor_rear)+'*L+-'+str(upmix_factor_rear)+'*R RR='+str(upmix_factor_rear)+'*R+-'+str(upmix_factor_rear)+'*L SL='+str(upmix_factor_side)+'*L+-'+str(upmix_factor_side)+'*R SR='+str(upmix_factor_side)+'*R+-'+str(upmix_factor_side)+'*L'
                f.write(copy_string)
                f.write('\n')
                f.write('\n')
            
            #section to apply specific channel gains
            f.write('Channel: L')
            f.write('\n')
            f.write('Preamp: '+ str(gain_fl) +' dB')#also adjust gain
            f.write('\n')
            f.write('Channel: R')
            f.write('\n')
            f.write('Preamp: '+ str(gain_fr) +' dB')#also adjust gain
            f.write('\n')
            if audio_channels == '7.1 Surround' or audio_channels == '5.1 Surround' or audio_channels == '2.0 Stereo Upmix to 7.1':
                f.write('Channel: C')
                f.write('\n')
                f.write('Preamp: '+ str(gain_c) +' dB')#also adjust gain
                f.write('\n') 
                f.write('Channel: SUB')
                f.write('\n')
                f.write('Preamp: '+ str(gain_c) +' dB')#also adjust gain
                f.write('\n') 
                f.write('Channel: RL')
                f.write('\n')
                f.write('Preamp: '+ str(gain_rl) +' dB')#also adjust gain
                f.write('\n')
                if audio_channels == '2.0 Stereo Upmix to 7.1':
                    f.write('Delay: '+ str(rear_delay) + ' samples')#also adjust delay
                    f.write('\n')
                f.write('Channel: RR')
                f.write('\n')
                f.write('Preamp: '+ str(gain_rr) +' dB')#also adjust gain
                f.write('\n')
                if audio_channels == '2.0 Stereo Upmix to 7.1':
                    f.write('Delay: '+ str(rear_delay) + ' samples')#also adjust delay
                    f.write('\n')
            if audio_channels == '7.1 Surround' or audio_channels == '2.0 Stereo Upmix to 7.1':
                f.write('Channel: SL')
                f.write('\n')
                f.write('Preamp: '+ str(gain_sl) +' dB')#also adjust gain
                f.write('\n')
                if audio_channels == '2.0 Stereo Upmix to 7.1':
                    f.write('Delay: '+ str(side_delay) + ' samples')#also adjust delay
                    f.write('\n')
                f.write('Channel: SR')
                f.write('\n')
                f.write('Preamp: '+ str(gain_sr) +' dB')#also adjust gain
                f.write('\n')
                if audio_channels == '2.0 Stereo Upmix to 7.1':
                    f.write('Delay: '+ str(side_delay) + ' samples')#also adjust delay
                    f.write('\n')
            f.write('\n')
            
            #downmixing if applicable
            if audio_channels == '7.1 Downmix to Stereo':
                copy_string = 'Copy: L=1*L+0.707*C+0.707*SUB+0.707*SL+0.707*RL R=1*R+0.707*C+0.707*SUB+0.707*SR+0.707*RR'
                f.write(copy_string)
                f.write('\n')
                f.write('\n')
                
            #section to perform brir convolution
            if enable_brir_conv == True:
                if audio_channels == '2.0 Stereo' or audio_channels == '7.1 Downmix to Stereo':
                    copy_string = 'Copy: L_INPUT_L_EAR=L L_INPUT_R_EAR=L R_INPUT_L_EAR=R R_INPUT_R_EAR=R'
                elif audio_channels == '7.1 Surround' or audio_channels == '2.0 Stereo Upmix to 7.1':
                    copy_string = 'Copy: L_INPUT_L_EAR=L L_INPUT_R_EAR=L R_INPUT_L_EAR=R R_INPUT_R_EAR=R C_INPUT_L_EAR=C+SUB C_INPUT_R_EAR=C+SUB RL_INPUT_L_EAR=RL RL_INPUT_R_EAR=RL RR_INPUT_L_EAR=RR RR_INPUT_R_EAR=RR SL_INPUT_L_EAR=SL SL_INPUT_R_EAR=SL SR_INPUT_L_EAR=SR SR_INPUT_R_EAR=SR'
                elif audio_channels == '5.1 Surround':
                    copy_string = 'Copy: L_INPUT_L_EAR=L L_INPUT_R_EAR=L R_INPUT_L_EAR=R R_INPUT_R_EAR=R C_INPUT_L_EAR=C+SUB C_INPUT_R_EAR=C+SUB RL_INPUT_L_EAR=RL RL_INPUT_R_EAR=RL RR_INPUT_L_EAR=RR RR_INPUT_R_EAR=RR'
                else:
                    copy_string = 'Copy: L_INPUT_L_EAR=L L_INPUT_R_EAR=L R_INPUT_L_EAR=R R_INPUT_R_EAR=R'
                f.write(copy_string)
                f.write('\n')
                f.write('Channel: L_INPUT_L_EAR L_INPUT_R_EAR')
                f.write('\n')
                f.write(conv_brir_rel_dir + brir_name_wav_fl)
                f.write('\n')
                f.write('Channel: R_INPUT_L_EAR R_INPUT_R_EAR')
                f.write('\n')
                f.write(conv_brir_rel_dir + brir_name_wav_fr)
                f.write('\n')
                if audio_channels == '7.1 Surround' or audio_channels == '5.1 Surround' or audio_channels == '2.0 Stereo Upmix to 7.1':
                    f.write('Channel: C_INPUT_L_EAR C_INPUT_R_EAR')
                    f.write('\n')
                    f.write(conv_brir_rel_dir + brir_name_wav_c)
                    f.write('\n')              
                    f.write('Channel: RL_INPUT_L_EAR RL_INPUT_R_EAR')
                    f.write('\n')
                    f.write(conv_brir_rel_dir + brir_name_wav_rl)
                    f.write('\n')
                    f.write('Channel: RR_INPUT_L_EAR RR_INPUT_R_EAR')
                    f.write('\n')
                    f.write(conv_brir_rel_dir + brir_name_wav_rr)
                    f.write('\n')
                if audio_channels == '7.1 Surround' or audio_channels == '2.0 Stereo Upmix to 7.1':
                    f.write('Channel: SL_INPUT_L_EAR SL_INPUT_R_EAR')
                    f.write('\n')
                    f.write(conv_brir_rel_dir + brir_name_wav_sl)
                    f.write('\n')
                    f.write('Channel: SR_INPUT_L_EAR SR_INPUT_R_EAR')
                    f.write('\n')
                    f.write(conv_brir_rel_dir + brir_name_wav_sr)
                    f.write('\n')

                if audio_channels == '2.0 Stereo' or audio_channels == '7.1 Downmix to Stereo':
                    copy_string = 'Copy: L=0*L+L_INPUT_L_EAR+R_INPUT_L_EAR R=0*R+L_INPUT_R_EAR+R_INPUT_R_EAR C=0*C RL=0*RL RR=0*RR SL=0*SL SR=0*SR SUB=0*SUB'
                elif audio_channels == '7.1 Surround' or audio_channels == '2.0 Stereo Upmix to 7.1':
                    copy_string = 'Copy: L=0*L+L_INPUT_L_EAR+R_INPUT_L_EAR+C_INPUT_L_EAR+RL_INPUT_L_EAR+RR_INPUT_L_EAR+SL_INPUT_L_EAR+SR_INPUT_L_EAR R=0*R+L_INPUT_R_EAR+R_INPUT_R_EAR+C_INPUT_R_EAR+RL_INPUT_R_EAR+RR_INPUT_R_EAR+SL_INPUT_R_EAR+SR_INPUT_R_EAR C=0*C RL=0*RL RR=0*RR SL=0*SL SR=0*SR SUB=0*SUB'
                elif audio_channels == '5.1 Surround':
                    copy_string = 'Copy: L=0*L+L_INPUT_L_EAR+R_INPUT_L_EAR+C_INPUT_L_EAR+RL_INPUT_L_EAR+RR_INPUT_L_EAR R=0*R+L_INPUT_R_EAR+R_INPUT_R_EAR+C_INPUT_R_EAR+RL_INPUT_R_EAR+RR_INPUT_R_EAR C=0*C RL=0*RL RR=0*RR SL=0*SL SR=0*SR SUB=0*SUB'
                f.write(copy_string)
                f.write('\n')
                f.write('\n')
                
            #section to perform hpcf convolution
            if enable_hpcf_conv == True:
                f.write('Channel: L R')
                f.write('\n')
                f.write(conv_hpcf_command)
                f.write('\n')
                f.write('\n')
  
    except Exception as ex:
        log_string = 'Failed to write config file: ' + out_file
        hf.log_with_timestamp(log_string=log_string, gui_logger=gui_logger, log_type = 1, exception=ex)#log warning
            
        return CN.EAPO_ERROR_CODE
            
    return preamp
    
    
def include_ash_e_apo_config(primary_path, enabled=False):
    """
    Function loads ASH_Toolset_Config equalizer APO configuration file in config.txt to perform relevant convolution commands.
    :param primary_path: string, base path to save files to
    :return: None
    """    
    status_code=0
    try:
        #EAPO config path
        output_config_path = pjoin(primary_path, CN.PROJECT_FOLDER_CONFIGS)
        
        #previously generated custom config file
        hesuvi_file_name = 'hesuvi.txt'
        custom_file_name = 'ASH_Toolset_Config.txt'
        custom_file = pjoin(output_config_path, custom_file_name)
        custom_file_rel = CN.PROJECT_FOLDER + '\\E-APO-Configs\\'+custom_file_name
        
        if os.path.isfile(custom_file):
        
            #config.txt file located in e-apo config directory
            e_apo_config_path = pjoin(primary_path, 'config.txt')
            
            if os.path.isfile(e_apo_config_path):
            
                #read from config.txt
                altered_lines = []
                include_custom_exists = 0
                with open(e_apo_config_path, "r", encoding='utf-8') as f:
                    for line in f.readlines():
                        altered_line=line
                        #comment out line if hesuvi config is active and so is ash toolset config
                        if hesuvi_file_name in line and '#' not in line and enabled == True:
                            altered_line = '# '+line
                        #comment out line if a previous format convolution config is active
                        if ('HpCF-Convolution' in line or 'BRIR-Convolution' in line) and '#' not in line and enabled == True:
                            altered_line = '# '+line
                        #if include custom config command already exists in config.txt
                        if custom_file_name in line:
                            include_custom_exists=1
                            #shouldnt be included (comment out)
                            #only if not already commented out
                            if enabled == False and '#' not in line: 
                                altered_line = '# '+line
                            #should be included (remove comment)
                            #only if already commented out
                            elif enabled == True and '#' in line: 
                                altered_line = line.replace("# ", "")
                            #rename config from old version (pre v2.4.0)
                            if CN.PROJECT_FOLDER_SSD in line: 
                                altered_line = altered_line.replace(CN.PROJECT_FOLDER_SSD, CN.PROJECT_FOLDER)
                                
                        altered_lines.append(altered_line)
                        
                        
                #write back to config.txt
                with open(e_apo_config_path, "w") as f:
                    #f.write('\n'.join(altered_lines) + '\n')
                    f.write(''.join(altered_lines))
                    #also write include line if enabled and not already included
                    if include_custom_exists == 0 and enabled == True:
                        if 'EqualizerAPO\\config\\'+ CN.PROJECT_FOLDER +'\\E-APO-Configs' in output_config_path:
                            include_string = 'Include: '+custom_file_rel#use relative directory if project folder is in same folder as config.txt
                        else:
                            include_string = 'Include: '+custom_file#use full path
                        f.write('\n' + include_string )
                    
                    
            else:
                raise ValueError('config.txt not modified. file not found: '+ str(e_apo_config_path))
    
        else:
            raise ValueError('config.txt not modified. file not found: '+ str(custom_file))
    
    except Exception as ex:
        hf.log_with_timestamp(f"Error: {ex}", log_type=2, exception=ex)  
        return CN.EAPO_ERROR_CODE
    
    return status_code
    
def get_exported_brir_list(primary_path):
    """
    Function reads BRIR folders in output directory and returns list of folders.
    :param primary_path: string, base path to save files to
    :return: list
    """    
    
    try:
        
        try:
            #find previous location (pre v2.4.0) and rename folder if found
            old_proj_path = pjoin(primary_path, CN.PROJECT_FOLDER_SSD)
            new_proj_path = pjoin(primary_path, CN.PROJECT_FOLDER)
            if os.path.exists(old_proj_path):
                #rename folder
                os.rename(old_proj_path, new_proj_path)
        except Exception as ex:
            logging.error("Error occurred", exc_info = ex)  
 
        brirs_path = pjoin(primary_path, CN.PROJECT_FOLDER_BRIRS)

        #list all names in path 
        directory_list = list()
        for root, dirs, files in os.walk(brirs_path, topdown=False):
            for name in dirs:
                if 'dB' in name:
                    extracted_brir_name =  name.replace("_", " ")
                    directory_list.append(extracted_brir_name)
        
        return sorted(directory_list, key=str.casefold)
    
    except Exception as ex:
        hf.log_with_timestamp(f"Error: {ex}", log_type=2, exception=ex)  
        return []
    
def get_spatial_res_from_dir(primary_path, brir_set):
    """
    Function reads BRIR folders in output directory and returns int containing estimated spatial resolution.
    :param primary_path: string, base path
    :param brir_set: string, name of brir set
    :return: int
    """  
    
    spatial_res=1#default value
    
    try:

        #find file names for desired brirs
        brir_set_formatted = brir_set.replace(" ", "_")
        brirs_path = pjoin(primary_path, CN.PROJECT_FOLDER_BRIRS, brir_set_formatted)

        spatial_res_low=0
        spatial_res_med=0 
        spatial_res_high=0 
        spatial_res_full=0
        
        #find specific directions to flag spatial res
        for root, dirs, files in os.walk(brirs_path):
            for filename in files:
                if '_E-50_' in filename:
                    spatial_res_high=1 
                if '_E-45_' in filename:
                    spatial_res_med=1 
                if '_E-30_' in filename:
                    spatial_res_low=1 
                if '_E-2_' in filename:
                    spatial_res_full=1     
  
        if spatial_res_full == 1:
            spatial_res=3
        elif spatial_res_high == 1:
            spatial_res=2
        elif spatial_res_med == 1:
            spatial_res=1
        elif spatial_res_low == 1:
            spatial_res=0

    except Exception as ex:
        hf.log_with_timestamp(f"Error: {ex}", log_type=2, exception=ex)   
        
    return spatial_res
    
def get_hrtf_type_from_dir(brir_set, spatial_res=1):
    """
    Function reads BRIR folders in output directory and returns int containing hrtf type.
    :param brir_set: string, name of brir set
    :param spatial_res: int, spatial resolution
    :return: int
    """  
    
    hrtf_type=1#default value
    hrtf_index=0
    
    try:

        #find hrtf type from file name
        brir_set_formatted = brir_set.replace(" ", "_")
        if spatial_res == 3:
            hrtf_list=CN.HRTF_A_LIST_FULL_RES_SHORT
        else:
            hrtf_list=CN.HRTF_A_LIST_SHORT
        for idx, x in enumerate(hrtf_list):
                hrtf_name_delim = x+'_'
                if hrtf_name_delim in brir_set_formatted:
                    hrtf_type=idx+1
                    hrtf_index=idx
    
    except Exception as ex:
        hf.log_with_timestamp(f"Error: {ex}", log_type=2, exception=ex)   
    
    return hrtf_type
    

    

    
def get_exported_hp_list(primary_path, delimiters, gui_logger=None):
    """
    Reads HpCF FIR folders and returns a distinct, sorted list of headphone names.
    Uses provided delimiters (e.g., sample names) to extract names before those substrings.
    Matching is case-insensitive, and separators ('_' or ' ') before delimiters are trimmed.
    Filenames that don't match any delimiter are logged via hf.log_with_timestamp.
    
    :param primary_path: str, base directory containing the HpCF 'FIRs' folder
    :param delimiters: list[str], list of substrings used to split filenames (e.g., ['SampleA', 'Average'])
    :param gui_logger: optional GUI logger object to pass to hf.log_with_timestamp
    :return: list[str], sorted unique list of headphone names
    """
    hpcf_fir_path = pjoin(primary_path, CN.PROJECT_FOLDER_HPCFS, "FIRs")
    hpcf_hp_list = []

    try:
        # Normalize delimiters: replace spaces with underscores, lowercase
        delimiters_norm = [d.replace(" ", "_").lower() for d in delimiters if d]

        for root, _, files in os.walk(hpcf_fir_path):
            for filename in files:
                if not filename.lower().endswith(".wav"):
                    continue

                # Remove file extension
                name = os.path.splitext(filename)[0]
                name_lower = name.lower()

                matched = False

                # Try to find and strip at the first matching delimiter (case-insensitive)
                for d_norm in delimiters_norm:
                    idx = name_lower.find(d_norm)
                    if idx != -1:
                        # Trim up to character *before* the matched delimiter
                        name = name[:idx].rstrip("_ ").strip()
                        matched = True
                        break

                # Replace underscores with spaces and tidy up
                name = name.replace("_", " ").strip()

                if name:
                    hpcf_hp_list.append(name)
                elif not matched and gui_logger:
                    log_string = f"Filename '{filename}' could not be matched to any delimiter."
                    hf.log_with_timestamp(log_string, gui_logger)
        
        # Debug print of final headphone list
        #print("Normalized delimiters:", delimiters_norm)
        #print("Extracted headphone list:", sorted(set(hpcf_hp_list), key=str.casefold))
        
        return sorted(set(hpcf_hp_list), key=str.casefold)

    except Exception as ex:
        hf.log_with_timestamp(f"Error: {ex}", log_type=2, exception=ex)
        return []
 


def est_peak_gain_from_dir(
    primary_path,
    brir_set,
    hpcf_dict,
    brir_meta_dict,
    gain_config=0,
    calc_mode=0,
    channel_config='2.0 Stereo',
    freq_mode='default'
):
    """
    Estimate peak gain from BRIRs and optional HpCF convolution.

    Args:
        primary_path (str): Base path to project folders.
        brir_set (str): Name of BRIR set.
        hpcf_dict (dict): Headphone correction FIR info.
        brir_meta_dict (dict): BRIR metadata dict.
        gain_config (float): Overall gain to add.
        calc_mode (int): 0=max, 1=mean
        channel_config (str): '2.0 Stereo', '5.1 Surround', '7.1 Surround'
        freq_mode (str): 'default', 'Align Low Frequencies', 'Align Mid Frequencies'

    Returns:
        float: Estimated peak gain (dB)
    """
    try:
        n_fft = CN.N_FFT
        # Use half-spectrum reference (RFFT)
        result_spectrum_l = np.copy(CN.FR_FLAT_DB_RFFT)
        result_spectrum_r = np.copy(CN.FR_FLAT_DB_RFFT)

        # --- Helper: convert gain/mute ---
        def get_channel_mag(gain_key, mute_key):
            gain = brir_meta_dict.get(gain_key, 0)
            if brir_meta_dict.get(mute_key, False):
                gain = -100
            return hf.db2mag(gain)

        gains_mag = {
            'fl': get_channel_mag('gain_fl', 'mute_fl'),
            'fr': get_channel_mag('gain_fr', 'mute_fr'),
            'c':  get_channel_mag('gain_c',  'mute_c'),
            'sl': get_channel_mag('gain_sl', 'mute_sl'),
            'sr': get_channel_mag('gain_sr', 'mute_sr'),
            'rl': get_channel_mag('gain_rl', 'mute_rl'),
            'rr': get_channel_mag('gain_rr', 'mute_rr')
        }
        
        # --- Zero out unused channels based on channel_config ---
        if channel_config == '2.0 Stereo':
            gains_mag['c']  = 0
            gains_mag['sl'] = 0
            gains_mag['sr'] = 0
            gains_mag['rl'] = 0
            gains_mag['rr'] = 0
        elif channel_config == '5.1 Surround':
            gains_mag['sl'] = 0
            gains_mag['sr'] = 0
        elif channel_config == '7.1 Surround':
            # all channels used, nothing to zero
            pass

        # --- Helper: BRIR filename ---
        def brir_fname(prefix, elev, azim):
            return f"{prefix}_E{elev}_A{azim}.wav"

        brir_files = {
            'fl': brir_fname('BRIR', brir_meta_dict.get('elev_fl'), brir_meta_dict.get('azim_fl')),
            'fr': brir_fname('BRIR', brir_meta_dict.get('elev_fr'), brir_meta_dict.get('azim_fr')),
            'c':  brir_fname('BRIR', brir_meta_dict.get('elev_c'),  brir_meta_dict.get('azim_c')),
            'sl': brir_fname('BRIR', brir_meta_dict.get('elev_sl'), brir_meta_dict.get('azim_sl')),
            'sr': brir_fname('BRIR', brir_meta_dict.get('elev_sr'), brir_meta_dict.get('azim_sr')),
            'rl': brir_fname('BRIR', brir_meta_dict.get('elev_rl'), brir_meta_dict.get('azim_rl')),
            'rr': brir_fname('BRIR', brir_meta_dict.get('elev_rr'), brir_meta_dict.get('azim_rr'))
        }

        # --- HpCF convolution ---
        if hpcf_dict.get('enable_conv', False):
            brand_fmt = hpcf_dict.get('brand', '').replace(" ", "_")
            hp_fmt = hpcf_dict.get('headphone', '').replace(" ", "_")
            sample_fmt = hpcf_dict.get('sample', '').replace(" ", "_")
            hpcf_path = pjoin(primary_path, CN.PROJECT_FOLDER_HPCFS, 'FIRs', brand_fmt,
                              f"{hp_fmt}_{sample_fmt}.wav")
            sr, data = hf.read_wav_file(hpcf_path)
            data_pad = hf.pad_or_truncate_1d(data, n_fft)
            output_fr_db = hf.mag2db(np.abs(np.fft.rfft(data_pad)))
            result_spectrum_l += output_fr_db
            result_spectrum_r += output_fr_db

        # --- BRIR convolution ---
        if brir_meta_dict.get('enable_conv', False):
            brir_set_fmt = brir_set.replace(" ", "_")
            brirs_path = pjoin(primary_path, CN.PROJECT_FOLDER_BRIRS, brir_set_fmt)

            brir_acc_l = np.zeros(n_fft)
            brir_acc_r = np.zeros(n_fft)

            for root, dirs, files in os.walk(brirs_path):
                for ch_key, fname in brir_files.items():
                    if fname in files:
                        wav_path = pjoin(root, fname)
                        sr, brir_data = hf.read_wav_file(wav_path)
                        fir_len = min(len(brir_data), n_fft)
                        brir_acc_l[:fir_len] += brir_data[:fir_len,0] * gains_mag[ch_key]
                        brir_acc_r[:fir_len] += brir_data[:fir_len,1] * gains_mag[ch_key]

            # --- Determine frequency range ---
            if freq_mode == CN.AUTO_GAIN_METHODS[2]:
                min_f, max_f = 10, 120
            elif freq_mode == CN.AUTO_GAIN_METHODS[3]:
                min_f, max_f = 800, 1200
            else:
                min_f, max_f = 10, 19000

            # --- FFT via rfft ---
            for ch_acc, res_spectrum in zip([brir_acc_l, brir_acc_r], [result_spectrum_l, result_spectrum_r]):
                mag = np.abs(np.fft.rfft(ch_acc[:n_fft]))
                mag = hf.level_spectrum_ends(mag, min_f, max_f, smooth_win=0)
                res_spectrum += hf.mag2db(mag)

        # --- Peak gain ---
        max_l = np.max(result_spectrum_l)
        max_r = np.max(result_spectrum_r)
        peak_gain = np.mean([max_l, max_r]) if calc_mode==1 else max(max_l, max_r)

        if hpcf_dict.get('enable_conv', False) or brir_meta_dict.get('enable_conv', False):
            peak_gain += gain_config

        return round(peak_gain, 1)

    except Exception as ex:
        hf.log_with_timestamp(f"Error in est_peak_gain_from_dir: {ex}", log_type=2, exception=ex)
        return 0.0  


def dataset_all_brirs_found(primary_path, brir_set, brir_meta_dict, channel_config='2.0 Stereo'):
    """
    Function reads BRIR folders in output directory and returns bool indicating if all BRIRs 
    were found AND are fresh based on timestamp comparison.
    """

    all_brirs_found = False
    try:
        enable_brir_conv = brir_meta_dict.get("enable_conv")

        # --- Build expected filenames ---
        elev_fl = brir_meta_dict.get("elev_fl")
        elev_fr = brir_meta_dict.get("elev_fr")
        elev_c  = brir_meta_dict.get("elev_c")
        elev_sl = brir_meta_dict.get("elev_sl")
        elev_sr = brir_meta_dict.get("elev_sr")
        elev_rl = brir_meta_dict.get("elev_rl")
        elev_rr = brir_meta_dict.get("elev_rr")

        azim_fl = brir_meta_dict.get("azim_fl")
        azim_fr = brir_meta_dict.get("azim_fr")
        azim_c  = brir_meta_dict.get("azim_c")
        azim_sl = brir_meta_dict.get("azim_sl")
        azim_sr = brir_meta_dict.get("azim_sr")
        azim_rl = brir_meta_dict.get("azim_rl")
        azim_rr = brir_meta_dict.get("azim_rr")

        wav_fl = f"BRIR_E{elev_fl}_A{azim_fl}.wav"
        wav_fr = f"BRIR_E{elev_fr}_A{azim_fr}.wav"
        wav_c  = f"BRIR_E{elev_c}_A{azim_c}.wav"
        wav_sl = f"BRIR_E{elev_sl}_A{azim_sl}.wav"
        wav_sr = f"BRIR_E{elev_sr}_A{azim_sr}.wav"
        wav_rl = f"BRIR_E{elev_rl}_A{azim_rl}.wav"
        wav_rr = f"BRIR_E{elev_rr}_A{azim_rr}.wav"

        # Determine expected BRIR count
        if channel_config in ("2.0 Stereo", "7.1 Downmix to Stereo"):
            expected_brirs = 2
            required_files = [wav_fl, wav_fr]
        elif channel_config == "5.1 Surround":
            expected_brirs = 5
            required_files = [wav_fl, wav_fr, wav_c, wav_rl, wav_rr]
        elif channel_config in ("7.1 Surround", "2.0 Stereo Upmix to 7.1"):
            expected_brirs = 7
            required_files = [wav_fl, wav_fr, wav_c, wav_sl, wav_sr, wav_rl, wav_rr]
        else:
            expected_brirs = 2
            required_files = [wav_fl, wav_fr]

        brirs_found = 0
        missing_files = []
        timestamp_mismatches = []

        if enable_brir_conv:
            brir_set_formatted = brir_set.replace(" ", "_")
            brirs_path = pjoin(primary_path, CN.PROJECT_FOLDER_BRIRS, brir_set_formatted)

            # --- Check each required file exists ---
            for fname in required_files:
                fpath = os.path.join(brirs_path, fname)
                if os.path.exists(fpath):
                    brirs_found += 1
                else:
                    missing_files.append(fname)

            # Log missing file summary
            if missing_files:
                hf.log_with_timestamp(
                    f"[BRIR Check] Missing {len(missing_files)}/{expected_brirs} files in set '{brir_set}': {missing_files}",
                    log_type=0
                )

            # Count mismatch = immediate fail
            if brirs_found < expected_brirs:
                hf.log_with_timestamp(
                    f"[BRIR Check] Found {brirs_found}/{expected_brirs} required BRIR files → FAIL",
                    log_type=0
                )
                return False

            # --- Timestamp freshness check ---
            try:
                ts_str = dpg.get_value("e_apo_sel_brir_set_ts")
                if ts_str not in (None, "", "0"):
                    run_ts = datetime.fromisoformat(ts_str)
                    tolerance = timedelta(seconds=10)

                    for fname in required_files:
                        fpath = os.path.join(brirs_path, fname)
                        if os.path.exists(fpath):
                            modified = datetime.fromtimestamp(os.path.getmtime(fpath))
                            if modified + tolerance < run_ts:
                                timestamp_mismatches.append((fname, modified))

                    if timestamp_mismatches:
                        for fname, ts in timestamp_mismatches:
                            hf.log_with_timestamp(
                                f"[BRIR Timestamp] File '{fname}' is older ({ts}) than run timestamp ({run_ts}) → MISMATCH",
                                log_type=0
                            )

                        hf.log_with_timestamp(
                            f"[BRIR Timestamp] {len(timestamp_mismatches)} file(s) failed timestamp freshness check → FAIL",
                            log_type=0
                        )
                        return False

            except Exception as ex:
                hf.log_with_timestamp(f"[BRIR Timestamp] Error checking timestamps: {ex}", log_type=2)

            # If we reached here: all files exist + timestamps match
            hf.log_with_timestamp(
                f"[BRIR Check] All {expected_brirs} files found and fresh for set '{brir_set}' → OK",
                log_type=0
            )
            all_brirs_found = True

        else:
            all_brirs_found = True

    except Exception as ex:
        hf.log_with_timestamp(f"Error: {ex}", log_type=2, exception=ex)

    return all_brirs_found